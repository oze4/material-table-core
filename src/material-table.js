    ...
    this.setState(
      {
        ...
        width: this.tableContainerDiv.current && this.tableContainerDiv.current.scrollWidth
      },
      () => {
      ...
      calculatedProps.actions = calculatedProps.actions
        ...
        .map((action) => {
          if (
            ...
            if (typeof action === 'function') { return { action: action, position: 'row' } } else return { ...action, position: 'row' };
          } else if (action.isFreeAction) {
            if (typeof action === 'function') { return { action: action, position: 'toolbar' } } else return { ...action, position: 'toolbar' };
          } else return action;
        });
    }
